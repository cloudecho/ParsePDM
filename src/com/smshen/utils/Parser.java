...
public class Parser {
	...
	public PDM pdmParser(String pdmFileName) throws Exception {
		...
		Document doc = reader.read(new File(pdmFileName));
		...
		LOGGER.info("解析PDM为:" + pdm.getCode() + "(" + pdm.getName() + ")  DBMS为:" + pdm.getDBMSCode() + "("
				+ pdm.getDBMSName() + ")");
	...
	public ArrayList<PDMTable> pdmTableParser(Node node) throws Exception {
		...
		for (Object o : l) {
			...
			for (Object index : tableNode.selectNodes("c:Indexes/o:Index")) {
				...
				/*
				 * for (Object column :
				 * indexNode.selectNodes("//c:Column/o:Column")) { String id =
	...
	public ArrayList<PDMColumn> pdmColumnParser(Node node) {
		...
		try {
			for (Object o : node.selectNodes("c:Columns/o:Column")) {
				...
				pdmColumn.setName(columnNode.selectSingleNode("a:Name") == null ? ""
						: columnNode.selectSingleNode("a:Name").getText());
				pdmColumn.setCode(columnNode.selectSingleNode("a:Code") == null ? ""
						: columnNode.selectSingleNode("a:Code").getText());
				pdmColumn.setDataType(columnNode.selectSingleNode("a:DataType") == null ? ""
						: columnNode.selectSingleNode("a:DataType").getText());
	...
	public ArrayList<PDMReference> pdmReferenceParser(Node node) throws Exception {
		...
		for (Object reference : node.selectNodes("c:References/o:Reference")) {
			...
			// 添加ParentTable
			String parentTableId = ((Element) referenceNode.selectSingleNode("c:ParentTable/o:Table"))
					.attributeValue("Ref");
			pdmReference.setParentTable(pdm.getPDMTable(parentTableId));
			// 添加ChildTable
			String childTableId = ((Element) referenceNode.selectSingleNode("c:ChildTable/o:Table"))
					.attributeValue("Ref");
